  ...
  return (
    <div className="text-center my-10">
      ...
      {connections && user && (connections.map((connection) => {
        ...
        const otherUser =
          fromId === loggedInId ? connection.toUserId : connection.fromUserId;

        const { name, age, photoUrl, gender, about, skills } = otherUser;
        return (
          <div key={connection._id} className=" flex gap-4 m-4 p-4 rounded-lg bg-base-200 w-1/2 mx-auto">
            <div className="my-1">
            <img className="w-20 h-20 rounded-full"alt="User-Img" src={photoUrl} />
            </div>
            <div className="text-left mx-4:">
            <h2 className="font-bold text-xl">{name}</h2>
            {age && gender && (<p className="font-medium text-shadow-white">{age+","+gender}</p>)}
